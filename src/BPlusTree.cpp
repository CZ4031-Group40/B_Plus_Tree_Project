...
BPNode* BPlusTree::findParentNode(BPNode* currentNode, BPNode* targetNode, BPNode* parentNode) {
    ...
    // Traverse the child nodes of the current node to find the targetNode
    for (int i = 0; i < currentNode->childNodePtrs.size(); i++) {
        if (currentNode->childNodePtrs[i] == targetNode) {
            return currentNode; // Found the parent node
        }

        // Recursively search for the targetNode in the child node
        BPNode* parent = findParentNode(currentNode->childNodePtrs[i], targetNode, currentNode);

        if (parent != nullptr) {
            return parent; // Propagate the parent node up the recursive call stack
        }
    }

    return nullptr; // targetNode not found in the current subtree
}



int BPlusTree::findIndexInParent(BPNode* parentNode, BPNode* childNode) {
    // Iterate through the childNodePtrs vector of the parentNode
    for (int i = 0; i < parentNode->childNodePtrs.size(); i++) {
        if (parentNode->childNodePtrs[i] == childNode) {
            return i; // Found the index of the childNode in the parent's childNodePtrs vector
        }
    }

    // Child node not found in the parent's childNodePtrs vector
    return -1; // You can choose to return -1 or some other suitable value to indicate not found
}
